        ...
        <div class="card-body">
            <form method="GET" class="row mb-3 align-items-end">
                ...
                <div class="col-md-3 mb-2 d-flex align-items-end flex-wrap gap-1">
                    ...
                    <a href="{{ route('inventory.brands.index') }}" class="btn btn-light btn-sm">Reset</a>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle">
                    <thead>
                        <tr><th>#</th><th>Name</th><th>Status</th><th class="text-right">Actions</th></tr>
                    </thead>
                    <tbody>
                        @forelse($brands as $brand)
                            <tr>
                                ...
                                <td class="text-right">
                                    <button type="button" class="btn-custom success" data-toggle="modal" data-target="#viewBrandModal{{ $brand->id }}"><i class="fa-solid fa-eye"></i></button>
                                    @can('inv_brand.edit')
                                        <button type="button" class="btn-custom yellow" data-toggle="modal" data-target="#editBrandModal{{ $brand->id }}"><i class="fa-solid fa-pen"></i></button>
                                    @endcan
                                    @can('inv_brand.delete')
                                        <button type="button" class="btn-custom danger" data-toggle="modal" data-target="#deleteBrandModal" data-action="{{ route('inventory.brands.destroy', $brand) }}"><i class="fa-solid fa-trash"></i></button>
                                    @endcan
                                </td>
                            </tr>
                                            ...
                                            <form method="POST" action="{{ route('inventory.brands.update', $brand) }}">
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="custom-control custom-switch mb-4">
                                                        ...
                                                        <input type="checkbox" name="is_active" value="1" class="custom-control-input" id="brandActive{{ $brand->id }}" @checked($brand->is_active)>
                                                        <label class="custom-control-label" for="brandActive{{ $brand->id }}">Active</label>
                                                    </div>
                                                </div>
                                                <div class="modal-footer">
            ...
            <div class="modal-content">
                <form method="POST" action="{{ route('inventory.brands.store') }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="custom-control custom-switch mb-4">
                            <input type="hidden" name="is_active" value="0">
                            <input type="checkbox" name="is_active" value="1" class="custom-control-input" id="brandActiveNew" checked>
                            <label class="custom-control-label" for="brandActiveNew">Active</label>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light btn-sm" data-dismiss="modal">Cancel</button>
