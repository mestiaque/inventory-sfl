...
<div class="flex-grow-1 p-4 inv-module">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0">Operators / Store Incharge</h4>
            <div>
        ...
        <div class="card-body">
            ...
            <form method="GET" class="row mb-3 align-items-end">
                ...
                <div class="col-md-3 mb-2">
                    <select name="designation" class="form-control form-control-sm inv-select2">
                ...
                <div class="col-md-3 mb-2 d-flex align-items-end flex-wrap gap-1">
                    ...
                    <a href="{{ route('inventory.operators.index') }}" class="btn btn-light btn-sm">Reset</a>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle">
                    <thead>
                        <tr><th>#</th><th>Name</th><th>Code</th><th>System Login</th><th>HR Employee</th><th>Designation</th><th>Assigned Store</th><th>Status</th><th class="text-right">Actions</th></tr>
                    </thead>
                    <tbody>
                        @forelse($operators as $operator)
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-{{ $operator->designation === 'operator' ? 'secondary' : 'info' }}">
                                        {{ ucwords(str_replace('_', ' ', $operator->designation)) }}
                                    </span>
                                </td>
                                <td>{{ $operator->store?->name ?? '—' }}</td>
                                <td>
                                    <span class="badge badge-{{ $operator->is_active ? 'success' : 'secondary' }}">
                                        {{ $operator->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <button type="button" class="btn-custom success" data-toggle="modal" data-target="#viewOperatorModal{{ $operator->id }}"><i class="fa-solid fa-eye"></i></button>
                                    @can('inv_operator.edit')
                                        <button type="button" class="btn-custom yellow" data-toggle="modal" data-target="#editOperatorModal{{ $operator->id }}"><i class="fa-solid fa-pen"></i></button>
                                    @endcan
                                    @can('inv_operator.delete')
                                        <button type="button" class="btn-custom danger" data-toggle="modal" data-target="#deleteOperatorModal" data-action="{{ route('inventory.operators.destroy', $operator) }}"><i class="fa-solid fa-trash"></i></button>
                                    @endcan
                                </td>
                            </tr>
