...
@section('title')
    @if($printMode)
        {{ websiteTitle('Style Wise Report') }}
    @else
        <title>{{ websiteTitle('Style Wise Report') }}</title>
    @endif
@endsection
...
<div class="flex-grow-1 inv-module">
    @if($printMode)
        @include('sfl-inventory::admin.reports.partials.print-header', ['title' => 'Style Wise Report'])
    @else
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Style Wise Report</h5>
            @unless($printMode)
                @include('sfl-inventory::admin.reports.partials.export-print-buttons', ['report' => 'style-wise'])
            @endunless
        </div>
        <div class="card-body">
            @unless($printMode)
            ...
            <h6>Summary — Received (Goods Receive by Style)</h6>
            <div class="table-responsive mb-2">
                <table class="table table-bordered table-striped table-sm align-middle">
                    <thead>
                        <tr>
                            <th>Style</th><th>Buyer(s)</th><th>Order Ref(s)</th>
                    ...
                    <tbody>
                        @forelse($receivedSummary as $row)
                            <tr>
                                <td>{{ $row->style }}</td>
                                <td>{{ $row->buyer_names ?: '—' }}</td>
                                <td>{{ $row->order_refs ?: '—' }}</td>
                        ...
                        @empty
                            <tr><td colspan="10" class="text-center text-muted">No receipts found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            <h6 class="mt-4">Detail — Received by Style</h6>
            <div class="table-responsive mb-2">
                <table class="table table-bordered table-striped table-sm align-middle">
                    <thead>
                        <tr>
                            <th>GRN No</th><th>Receive Date</th><th>Style</th><th>Buyer</th><th>Supplier</th><th>Order Ref</th>
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr><td colspan="21" class="text-center text-muted">No receipts found.</td></tr>
                        @endforelse
                    </tbody>
                    @if($received->isNotEmpty())
                        <tfoot>
                            <tr class="fw-bold"><td colspan="12" class="text-end">Total</td><td class="text-end">{{ inv_qty($received->sum('received_qty')) }}</td><td></td><td class="text-end">{{ inv_qty($received->sum('amount')) }}</td><td colspan="6"></td></tr>
                        </tfoot>
                    @endif
                </table>
            </div>
            ...
            <h6 class="mt-4">Summary — Issued by Style</h6>
            <div class="table-responsive mb-2">
                <table class="table table-bordered table-striped table-sm align-middle">
                    <thead>
                        <tr>
            ...
            <h6 class="mt-4">Detail — Issued by Style</h6>
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-sm align-middle">
                    <thead>
                        <tr>
                            <th>Issue No</th><th>Issue Date</th><th>Style</th><th>Buyer</th><th>Order Ref</th><th>Department</th>
                            <th>Item Code</th><th>Item Name</th><th>Color</th><th>Size</th><th>Store</th><th>Unit</th>
