...
<div class="flex-grow-1 inv-module">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="mb-0">Buyer &amp; Style Wise Report</h4>
            @unless($printMode)
        ...
        <div class="card-body">
            ...
            <style>
                .inv-grouped-table > tbody > tr.inv-group-start > td { border-top: 2px solid #6c757d; }
                .inv-grouped-table > tbody > tr > td[rowspan] { background: #fff8ed; vertical-align: middle; }
            </style>
            <div class="table-responsive">
                <table class="table table-bordered table-sm align-middle inv-grouped-table">
                    <thead>
                        <tr>
                            <th>Style</th><th>Buyer</th><th style="min-width:200px">Item</th><th>Unit</th><th>Color</th><th>Size</th>
                            <th class="text-right">Received Qty</th><th class="text-right">Rejected Qty</th><th class="text-right">Rate</th><th class="text-right">Amount</th>
                            <th>Lot No</th><th>Batch No</th><th>Expiry Date</th>
                            <th class="text-right">Issue/Delivery Qty</th><th class="text-right">Balance</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($rows as $i => $line)
                            <tr @class(['inv-group-start' => $styleSpans[$i] > 0 && $i > 0])>
                                @if($styleSpans[$i] > 0)
                                    <td rowspan="{{ $styleSpans[$i] }}" class="font-weight-bold align-middle">{{ $line->grn?->style ?: '—' }}</td>
                                @endif
                                @if($buyerSpans[$i] > 0)
                                ...
                                <td class="text-right">{{ inv_qty($line->rejected_qty) }}</td>
                                ...
                                <td class="text-right">{{ inv_qty($line->item_total_issued) }}</td>
                                <td class="text-right">
                                    <span class="badge badge-{{ $line->item_balance > 0 ? 'success' : ($line->item_balance < 0 ? 'danger' : 'secondary') }}">{{ inv_qty($line->item_balance) }}</span>
                                </td>
                            </tr>
                        @empty
                    ...
                    @if($rows->isNotEmpty())
                        <tfoot>
                            <tr class="font-weight-bold">
                                ...
                                <td class="text-right">{{ inv_qty($rows->sum('received_qty')) }}</td>
                                <td class="text-right">{{ inv_qty($rows->sum('rejected_qty')) }}</td>
                                <td></td>
                                <td class="text-right">{{ inv_qty($rows->sum('amount')) }}</td>
                                <td colspan="5"></td>
                            </tr>
                        </tfoot>
                    @endif
