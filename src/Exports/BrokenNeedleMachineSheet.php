...
class BrokenNeedleMachineSheet implements FromArray, ShouldAutoSize, WithEvents, WithTitle
{
    ...
    public function array(): array
    {
        $data = [
            ...
            [$this->periodLabel()],
            [''],
            ['#', 'Machine', 'Code', 'Department', 'Section', 'Line', 'Incidents', 'Total Broken Needle Qty'],
        ];

        foreach ($this->rows as $i => $row) {
            $data[] = [
                $i + 1,
                $row->machine?->name ?? 'Not Specified',
                $row->machine?->code ?? '—',
                $row->machine?->department?->name ?? '—',
                $row->machine?->section ?? '—',
                $row->machine?->line ?? '—',
                (int) $row->incidents,
                (int) $row->total_qty,
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $this->applyReportStyles($event->sheet->getDelegate(), 8, 4, $lastRow);
            },
        ];
    }
